...
def add_delta_features(features_df):
    ...
    for col in df.columns:
            ...
            delta_features[f'{col}_rolling_avg_change3'] = df[col].rolling(window=3).mean().diff()
...
X = price_factors_df.drop(columns=['Price', 'Date', 'target', 'future_price', 'price_change', 'price_ma'])
X = X.ffill().bfill()
...
with open(f"{folder_path}/forward_features_selection_list_24h_long_0_8_10", 'r') as file:
...
##### correlation map
#corr_matrix = X_train_list.corr()
#high_corr = corr_matrix[(corr_matrix.abs() > 0.8) & (corr_matrix < 1)]
#high_corr_pairs = high_corr.stack().reset_index()
#high_corr_pairs.columns = ["Feature 1", "Feature 2", "Correlation"]
#high_corr_pairs = high_corr_pairs.sort_values(by="Correlation", ascending=False, key=abs)
#for index, row in high_corr_pairs.iterrows():
#    print(f"{row['Feature 1']} vs {row['Feature 2']}  {row['Correlation']:.2f}")
...
def run_model(X_train, X_test, y_train, y_test, train_price_date, test_price_date, num_boost_round=10000):
    ...
    #output['Train score'] = report_train['1']['precision']
...
def evaluate_feature(feature_name):
    ...
    if score > 0.82777 and num_boost_round > 0:
        #if cv_results.loc[num_boost_round, 'aucpr_gap'] > 0.05 and cv_results.loc[num_boost_round, 'aucpr_gap'] < 0.065:
        if cv_results.loc[num_boost_round, 'aucpr_gap'] < 0.05981745:
            ...
            output['aucpr_gap'] = cv_results.loc[num_boost_round, 'aucpr_gap']
            if output['Train Return'] > 0.1 and output['Test Return'] > 0.1 and num_boost_round > 10:
                print(output)
                # Save a full model
                ...
                model_combine_99.save_model(f"24h_long_models/{feature_name_replace}.bin")
                return 'good', output
...
sorted_output_single_feature_list = pd.DataFrame(sorted(output_single_feature_list, key=lambda x: x["Test Sharpe"], reverse=True))
...
model_99.save_model("20250226_xgboost_24h_0_8_0.bin")
print(output_99)
