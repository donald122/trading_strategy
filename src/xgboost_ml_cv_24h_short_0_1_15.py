...
with open(f"{folder_path}/forward_features_selection_list_24h_short_0_1_1", 'r') as file:
...
if __name__ == '__main__':
    ...
    def evaluate_feature(feature_name):
        ...
        if score > 0.636009 and num_boost_round > 0:
            if cv_results.loc[num_boost_round, 'aucpr_gap'] < 0.120:
                ...
                output['aucpr_gap'] = cv_results.loc[num_boost_round, 'aucpr_gap']
                if output['Train Return'] > 0.1 and output['Test Return'] > 0.1 and output['nround'] > 20 and output['Test precision'] > 0.73 and output['Test score'] > 0.569:
                    print(output)
                    # Save a full model
