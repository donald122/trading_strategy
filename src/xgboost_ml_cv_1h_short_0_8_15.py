...
INTERVAL = '1h'
DELAY = 2
# the following 2 has to be at least 1
LOOK_AHEAD = 4
MA = 1
THRESHOLD = 0.504
...
factors_df = factors_df[['/v1/metrics/distribution/balance_paypal-v-0']]
...
with open(f"{folder_path}/forward_features_selection_list_1h_short_0_0_0", 'r') as file:
...
if __name__ == '__main__':
    ...
    def evaluate_feature(feature_name):
        ...
        if score > 0.3 and num_boost_round > 0:
            if cv_results.loc[num_boost_round, 'aucpr_gap'] < 0.15:
                ...
                output['aucpr_gap'] = cv_results.loc[num_boost_round, 'aucpr_gap']
                if output['Train Return'] > 0.03 and output['Test Return'] > 0.03 and output['Test score'] > 0 and output['Test precision'] > 0.3 and output['Test score'] > 0.3:
                    print(output)
                    # Save a full model
                    ...
                    model_combine_99.save_model(f"1h_short_models/{feature_name_replace}.ubj")
                    return 'good', output
