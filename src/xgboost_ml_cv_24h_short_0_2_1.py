...
with open(f"{folder_path}/forward_features_selection_list_24h_short_0_2_1", 'r') as file:
...
xgb_default_params = {
    ...
    #'colsample_bylevel': 0.75,         # Feature sampling
    #'n_jobs': -1,
    #'alpha': 1,
    #'lambda': 2,
    'min_child_weight': 5,
...
if __name__ == '__main__':
    ...
    def evaluate_feature(feature_name):
        ...
        if score > 0.6955 and num_boost_round > 0:
            if cv_results.loc[num_boost_round, 'aucpr_gap'] < 0.116:
                ...
                output['aucpr_gap'] = cv_results.loc[num_boost_round, 'aucpr_gap']
                if output['Train Return'] > 0.1 and output['Test Return'] > 0.1 and output['nround'] > 20 and output['Test precision'] > 0.5 and output['Test aucpr'] > 0.7514:
                    print(output)
                    # Save a full model
                    ...
                    model_combine_99.save_model(f"24h_short_models/{feature_name_replace}.ubj")
                    return 'good', output
