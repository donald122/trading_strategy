...
with open(f"{folder_path}/forward_features_selection_list_24h_0_8_0", 'r') as file:
...
def evaluate_feature(feature_name):
    ...
    if score > 0.75208 and num_boost_round > 0:
        if cv_results.loc[num_boost_round, 'aucpr_gap'] < 0.122:
            ...
            output['aucpr_gap'] = cv_results.loc[num_boost_round, 'aucpr_gap']
            if output['Train Return'] > 0.1 and output['Test Return'] > 0.1 and output['Test score'] > 0.660:
                print(output)
                # Save a full model
                ...
                model_combine_99.save_model(f"24h_short_models/{feature_name_replace}.bin")
                return 'good', output
