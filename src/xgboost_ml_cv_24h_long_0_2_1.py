...
#factors_df = factors_df[['/v1/metrics/addresses/accumulation_count_pit-v-0', '/v1/metrics/supply/rcap_by_date_bands-o-2024', '/v1/metrics/indicators/stock_to_flow_ratio-o-daysTillHalving',
#                         '/v1/metrics/transactions/transfers_volume_miners_to_exchanges_all_pit-volumes-PegaPool', '/v1/metrics/transactions/transfers_volume_miners_to_exchanges_all-volumes-NovaBlock', '/v1/metrics/transactions/transfers_volume_miners_to_exchanges_all-volumes-PegaPool']]
...
with open(f"{folder_path}/forward_features_selection_list_24h_long_0_2_1", 'r') as file:
...
xgb_default_params = {
    ...
    'eta': 0.0025, #0.0026                    # Learning rate
    'max_depth': 7, #7                 # Max depth of trees
...
def run_model(X_train, X_test, y_train, y_test, train_price_date, test_price_date, num_boost_round=10000):
    ...
    y_train_pred_signal = y_train_pred_binary
    y_pred_binary = (y_pred  > THRESHOLD).astype(int)
    y_pred_signal = y_pred_binary
    ...
    output['Train precision'] = report_train['1']['precision']
...
#        continue
#    #output, _, _ = run_model(X_train_dropped, X_test_dropped, y_train, y_test, train_price_date, test_price_date, 496)
...
#sorted_output_list = pd.DataFrame(sorted(output_list, key=lambda x: x["CV score"], reverse=True))
...
if __name__ == '__main__':
    ...
    def evaluate_feature(feature_name):
        ...
        if score > 0.5725642 and num_boost_round > 0:
            if cv_results.loc[num_boost_round, 'aucpr_gap'] < 0.120:
                ...
                output['aucpr_gap'] = cv_results.loc[num_boost_round, 'aucpr_gap']
                if output['Train Return'] > 0.1 and output['Test Return'] > 0.1 and output['nround'] > 20 and output['Test precision'] > 0.52 and output['Test aucpr'] > 0.603760652:
                    print(output)
                    # Save a full model
    ...
    results = Parallel(n_jobs=-1)(delayed(evaluate_feature)(col_name) for col_name in tqdm(features, desc="Processing Features", unit="feature"))
    ...
    #output, report, model = run_model(X_train_list, X_test_list, y_train, y_test, train_price_date, test_price_date, 629)
    ...
    #output_99, _, model_99 = run_model(X_train_list_99, X_test_list_99, y_train_99, y_test_99, train_price_date_99, test_price_date_99, 629)
