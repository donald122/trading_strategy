...
with open(f"{folder_path}/forward_features_select_lightgbm_list_filtered", 'r') as file:
...
lgb_model = LGBMClassifier(max_depth=8, n_jobs=-1)
...
non_zero_importances = feature_importance_df[feature_importance_df['Importance'] > 2]
...
#with open(f"{folder_path}/forward_features_select_lightgbm_list", 'w') as f:
