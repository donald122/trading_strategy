...
btc_price_df['Price'] = btc_price_df['Price'].shift(-6)
...
price_factors_df = price_factors_df.replace([np.inf, -np.inf], 0)
...
price_factors_df['smoothed_signal'] = price_factors_df['signal'].rolling(window=1).apply(rolling_mode, raw=False).shift(0)
...
# select features in the list
with open(f"{folder_path}/forward_features_selection_list", 'r') as file:
...
model = RandomForestClassifier(verbose=True, n_jobs=-1, n_estimators=600, max_depth=8, min_samples_split=3, min_samples_leaf=4, class_weight='balanced')
